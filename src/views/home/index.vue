<template>
  <div :class="prefixCls">
    ...
    <div :class="{'unscrollable':showAddrFlag}">
      ...
      <!-- 选择收货地址 -->
      <div v-if="showAddrFlag">
        <transition name="slide-fade">
          <Address @getAddrObjFromShowAddr="getAddrObjFromShowAddr"></Address>
        </transition>
      </div>
...
export default {
  ...
  methods: {
    ...
    getAddrObjFromShowAddr(addr, addrFlag, city) { // 从“选择收货地址”获取到地址对象
      this.showAddrFlag = false; // 不显示“收货地址”组件
      console.log('ok to home, get data from addr comp, addr&city=', addr, city);
      this.city = city;
      this.address = addr;
      if (addrFlag) { // 如果获取到的地址对象不为空
        this.addrFlag = true;
        console.log('ok to get addr&city in address component, this.city&address=', this.city, this.address);
      } else {
        this.address = "未能获取地址";
        this.addrFlag = false;
        console.log('err for back to home to get addr, address=', this.address);
      }
    }
  },
  created() {
    let storeAddr = this.$store.getters;
    console.log('home component, store.state.user.address----', this.$store.state.user.address);
    console.log('store----', this.$store.getters);
    if (storeAddr.city) { // 如果store中的address存在，将里面的address取出来，并将address标为已存在
      ...
      console.log('addrFlag--', this.addrFlag, ',address--', this.address,',city--', this.city);
    }
    // 有用户信息（登陆后）
    if (this.user) {
      // 请求商家数据
    ...
    console.log('when created, addrFlag--', this.addrFlag);
  },
  mounted() {
    this.scrollGoTop();
