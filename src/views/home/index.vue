...
export default {
  ...
  created() {
    let storeAdrr = this.$store.state.user.address;
    if (storeAdrr) { // 如果store中的address存在，将里面的address和city取出来
      this.address = storeAdrr.address;
      this.city = storeAdrr.city;
    }
    ...
    if (this.user) {
      // 请求商家数据
      this.$axios.get('/store/save').then(
        res => {
          this.stores = res;
        },
        err => {
